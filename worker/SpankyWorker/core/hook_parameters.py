...
def extract_params(s):
    ...
    has_defval = False
    for var in decls:
        if decls[var]["default"] is not None:
            has_defval = True
        elif has_defval:
            raise ValueError(
...
def map_params(s, params, strict=False):
    ...
    # Remove last word if it's an emoji
    if s.endswith(">"):
        s = s[0 : s.rfind("<")]
    ...
    while idx_in < len(input_toks) and idx_out < len(output_vals):
        ...
        if params[param]["type"] == "int":
            output_vals[param] = int(args)
        elif params[param]["type"] == "float":
            output_vals[param] = float(args)
        else:
    ...
    # Strict means that all parameters should be defined
    if strict:
        for key, value in output_vals.items():
            if not value:
                raise ValueError(f"Could not parse input for `{key}`. \
