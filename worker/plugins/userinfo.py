...
async def userinfo(text, reply, event):
    ...
    # join timestamp
    target_member = None
    for member in event.server.get_users():
        if member.id == id:
    ...
    output += f"Join date: {target_member.joined_at.strftime(dateString)}\n"
    output += f"Avatar: {target_member.avatar_url}\n"
    output += f"ID: {id}\n"

    if target_member.premium_since != None:
        output += (
            f"Boosting since \{target_member.premium_since.strftime(dateString)}\n"
        )
...
@hook.command()
async def inrole(text, event, reply):
    """
    [role name] List how many members each role has.
    Calling it with no role name will list all roles
    """
    role_id = {}
    total_members = 0
    ...
    alphab_sroles = {}
    for server_role in event.server.roles:
