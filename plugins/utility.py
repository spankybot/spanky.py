...
@hook.command(name="base64")
def base64_encode(text):
    """<string> -- Encode <string> with base64."""
    return base64.b64encode(text.encode()).decode()


@hook.command(name="debase64")
...
def leet(text):
    ...
    output = "".join(
        random.choice(leet[ch]) if ch.isalpha() else ch for ch in text.lower()
    )
    return output
