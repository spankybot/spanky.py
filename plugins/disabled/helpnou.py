...
def help(bot, text, event, send_embed, reply):
    ...
    reply("Comenzi bot:\nComenzi generale - <https://mdmd.kilonova.ro/spankyfork/comenzi/%s/commands>\nComenzi administrative - <https://mdmd.kilonova.ro/spankyfork/comenzi/%s/admin>" % (event.server.id, event.server.id))
    return
...
def send_off():
    return subprocess.check_output("rsync -avP %s/ webServer:%s/" % (outDir, vropage), shell=True).decode("utf-8")
