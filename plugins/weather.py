...
@hook.command()
def weather(text, reply, db, event):
    ...
    if not text:
        location = get_location(event.author.name)
        if not location:
    ...
    reply("{place} - Current: {conditions}, {temp_f}F/{temp_c}C, {humidity}, "
          "Wind: {wind_mph}MPH/{wind_kph}KPH {wind_direction}, Today: {today_conditions}, "
          "High: {today_high_f}F/{today_high_c}C, Low: {today_low_f}F/{today_low_c}C. "
          "Tomorrow: {tomorrow_conditions}, High: {tomorrow_high_f}F/{tomorrow_high_c}C, "
          "Low: {tomorrow_low_f}F/{tomorrow_low_c}C - {url}".format(**weather_data))
    if text:
        add_location(event.author.name, location, db)
