...
def gulag(send_message, text, server, event, bot, str_to_id, send_embed):
 ...
 The abbrebiations are: s - seconds, m - minutes, h - hours, d - days.
    ...
    if reason:
       ...
       send_embed("User gulaged", "", {"Details": gulag_text}, target="#mod-actions")
    send_message(ret)
...
@hook.command(permissions=Permission.admin, server_id=EUROPE_ID)
def confine(send_message, text, server, event, bot, str_to_id, send_embed):
 ...
 The abbrebiations are: s - seconds, m - minutes, h - hours, d - days.
    ...
    if reason:
        ...
        send_embed("User confined", "", {"Details": gulag_text}, target="#mod-actions")
    send_message(ret)

@hook.command(permissions=Permission.admin, server_id=EUROPE_ID)
def furrygulag(send_message, text, server, event, bot, str_to_id, send_embed):
    """<user, duration> - assign Furry Inmate role for specified time - duration can be seconds, minutes, hours, days.\
 To set a 10 minute 15 seconds timeout for someone, type: '.furrygulag @user 10m15s'.\
 The abbrebiations are: s - seconds, m - minutes, h - hours, d - days.
    """
    ret, reason = assign_temp_role(rstorage, roddit, bot, "Furry Inmate", text, "furrygulag", str_to_id, event)
    ...
    if reason:
        ...
        send_embed("User furrygulaged", "", {"Details": gulag_text}, target="#mod-actions")
    send_message(ret)
